...
class LHLoss(nn.Module):
    ...
    def logsum(self, x, alpha, axes):
        """Compute logsumexp over specified axes, preserving other dimensions."""
        return torch.log(torch.sum(torch.exp(x/alpha), dim=axes, keepdim=False))*alpha
        # return torch.logsumexp(x / alpha, dim=axes, keepdim=False) * alpha
    
    def logsum2(self, x, y, alpha):
        """Compute logsumexp of two tensors element-wise."""
        # return torch.log(torch.exp(x/alpha)+torch.exp(y/alpha))*alpha
        return (x+y+torch.abs(x-y))/2
    
    
    def logsum3(self, x, y, alpha):
        """Compute logsumexp of two tensors element-wise."""
        return torch.log(torch.exp(x/alpha)+torch.exp(y/alpha))*alpha

    def forward(self, x: torch.Tensor, y: torch.Tensor, loss_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """  Forward pass for LH loss computation."""
        shp_x = x.shape
        # breakpoint()
        ...
        # Compute logsumexp per channel over spatial (and optionally batch) dimensions
        fp = self.logsum(dm_gt * x_for_loss, self.alpha, axes) 
        fn = self.logsum(dm_hat * y_onehot_for_loss, self.alpha, axes)
        
        
        
        # if self.norm == 0:
        #     fp = fp/(x.sum(axes) + self.eps)
        #     fn = fn/(y_onehot.sum(axes) + self.eps)
        # elif self.norm == 1:
        #     denominator = (((x.sum(axes)) + (y_onehot.sum(axes)))/2) + self.eps
        #     fp = fp/denominator
        #     fn = fn/denominator
        # elif self.norm == 2:
        #     denominator = (x * y_onehot.float()).sum(axes) + self.eps
        #     fp = fp/denominator
        #     fn = fn/denominator
        # elif self.norm == 3:
        #     denominator = y_onehot.sum(axes) + self.eps
        #     fp = fp/denominator
        #     fn = fn/denominator

        # Combine fp and fn per channel
        lh_loss = self.logsum2(fp, fn, self.alpha)
