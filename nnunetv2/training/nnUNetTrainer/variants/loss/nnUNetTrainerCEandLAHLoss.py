import numpy as np
from nnunetv2.training.loss.compound_losses import CE_and_LAH_loss
...
class nnUNetTrainerCEandLAHLoss(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, device)
        self.initial_weight_lah = 1.0
        self.weight_ce = 1.0
        self.lah22_kwargs = {'iters': 2, 'connec': 26, 'tan': False, 'norm': 0, 'do_bg': self.label_manager.has_regions,
